...
def load_sample_data():
    file1 = './data/list_a.txt'
    file2 = './data/list_b.txt'
    file3 = './data/list_c.txt'
    try:
        with open(file1, 'r') as f: list_a = f.read()
        with open(file2, 'r') as f: list_b = f.read()
        with open(file3, 'r') as f: list_c = f.read()
        ...
        st.error(f'Sample data file does not exist. Pleasure ensure list_{{a,b,c}}.txt are in the same directory as your app.')
    return None
...
def process_text_area(text):
    # 1. Clean the text by removing quotes
...
# Initializes session state for text area 
if 'list1_content' not in st.session_state: st.session_state.list1_content = ''
if 'list2_content' not in st.session_state: st.session_state.list2_content = ''
if 'list3_content' not in st.session_state: st.session_state.list3_content = ''
...
# Navigate and sample data buttons 
with st.sidebar:
...
if page == '2 Lists':
    ...
    if (list1 != []) and (list2 != []):
        ...
        if st.button('Common elements'):
            ...
            st.write('Eelements: ', set(common_elements))
        # List differences
        list1 = set(list1)
        list2 = set(list2)
    ...
    else:   st.info('☝️ Enter data to proceed!')
...
if page == '3 Lists':
    ...
    if (list1 != []) and (list2 != []) and (list3 != []):
        ...
        common_all = set1.intersection(set2,set2)
        ...
        download_data(common_all, 'Common elements (all lists)')
        download_data(common_1_2, 'Common elements (Lists 1 & 2)')
        download_data(common_1_3, 'Common elements (Lists 1 & 3)')
        download_data(common_2_3, 'Common elements (Lists 2 & 3)')
    else: st.info('☝️ Enter data to proceed!')